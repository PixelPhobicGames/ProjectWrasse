...
auto LoadWorld()
{
    ...
    if (OmegaTechData.Deaths != 3)
    {
        ...
        {
            float Red = float((int(PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/AmbientLight.conf", OmegaTechData.LevelIndex) , 0)) / 255));
            float Green = float((int(PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/AmbientLight.conf", OmegaTechData.LevelIndex) , 1)) / 255));
            float Blue = float((int(PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/AmbientLight.conf", OmegaTechData.LevelIndex) , 2)) / 255));
            float Alpha = float((int(PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/AmbientLight.conf", OmegaTechData.LevelIndex) , 3)) / 255));
        ...
        {
            WDLModels.HeightMapW = PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/HeightMap.conf", OmegaTechData.LevelIndex) , 0);
            WDLModels.HeightMapH = PullConfigValue(TextFormat("GameData/Worlds/World%i/Config/HeightMap.conf", OmegaTechData.LevelIndex) , 1);
...
void PutLight(Vector3 Position){
    OmegaTechData.GameLights[LightCounter] = CreateLight(LIGHT_DIRECTIONAL, Position, Vector3Zero(), RED, OmegaTechData.Lights);
    LightCounter ++;
}
...
void OmegaTechInit()
{
    ...
    OmegaTechData.FinalShader = LoadShader(0, "GameData/Shaders/Final.fs");
...
void PlayHomeScreen()
{
    ...
    while (true && !WindowShouldClose())
    {
        ...
        if (GuiButton(LayoutRecs[0], "Start"))
        {
        ...
        if (GuiButton(LayoutRecs[1], "Load"))
        {
            UnloadRenderTexture(Target);
        ...
        if (GuiButton(LayoutRecs[3], "Setting"))
        {
            if (MenuSettings)
            {
                MenuSettings = false;
...
void UpdatePlayer()
{
    ...
    if (OmegaTechData.MainCamera.position.z >= 0 && OmegaTechData.MainCamera.position.z <= WDLModels.HeightMapW){
        if (OmegaTechData.MainCamera.position.x >= 0 && OmegaTechData.MainCamera.position.x <= WDLModels.HeightMapW){

            if (IsKeyDown(KEY_W) || IsKeyDown(KEY_A) || IsKeyDown(KEY_S) || IsKeyDown(KEY_D)){
                OmegaTechData.MainCamera.position.y = 8 + TerrainHeightMap[int(OmegaTechData.MainCamera.position.z)][int(OmegaTechData.MainCamera.position.x)];
            }
        }
    }
}
