...
auto LoadWorld() {
    if (OmegaTechData.LevelIndex != 3) {
        ...
        if (IsPathFile(TextFormat("GameData/Worlds/World%i/Entities/Walker/Frame1.png", OmegaTechData.LevelIndex))) {
            EnemyTextures.Frame1 =
                LoadTexture(TextFormat("GameData/Worlds/World%i/Entities/Walker/Frame1.png", OmegaTechData.LevelIndex));
            EnemyTextures.Scream =
                LoadSound(TextFormat("GameData/Worlds/World%i/Entities/Walker/Scream.mp3", OmegaTechData.LevelIndex));
        ...
        if (IsPathFile(TextFormat("GameData/Worlds/World%i/Models/HeightMap.png", OmegaTechData.LevelIndex))) {
            ...
            WDLModels.ShadowMap = LoadModel(TextFormat("GameData/Worlds/World%i/Models/Terrain.obj", OmegaTechData.LevelIndex));
            WDLModels.ShadowMap.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = ShadowTarget.texture;
            WDLModels.ShadowMap.materials[0].shader = OmegaTechData.Lights;
        ...
        if (IsPathFile("GameData/Global/FModels/FModel1.gltf")) {
            ...
            if (WDLModels.FastModel1.meshCount != 0) {
                UnloadModel(WDLModels.FastModel1);
            }
        }
...
void UpdateLightSources() {
    ...
    OmegaTechData.GameLights[0].position = {400, 500, 400};
    OmegaTechData.GameLights[0].target = {400, 32, 400};
...
void PlayFMV() {
    ...
    while (true && !WindowShouldClose()) {
        ...
        if (FMVVideo.video_state == RAY_VIDEO_STATE_DONE || (IsKeyPressed(KEY_SPACE))) {
            ...
            ShadowTarget = LoadRenderTexture(800 , 800);
...
void CWDLProcessShadows() {
    ...
    for (int i = 0; i <= CachedModelCounter; i++) {
        ...
        switch (CachedModels[i].ModelId) {
            case 2:
                BeginTextureMode(ShadowTarget);{
                    BeginMode3D(OmegaTechData.ShadowCamera);
                    {
...
void CWDLProcess() {
    ...
    for (int i = 0; i <= CachedModelCounter; i++) {
        ...
        case -1:
            ...
            DrawModelEx(WDLModels.ShadowMap, {X, Y ,Z}, {0, Rotation, 0}, Rotation, {S, S, S}, WHITE);
            //EndBlendMode();
            break;
        case 1:
...
void DrawWorld() {
    if (!ShadowsCached){
        ...
        Image Temp = LoadImageFromTexture(ShadowTarget.texture);   
        ImageFlipVertical(&Temp);     
        UpdateTexture(ShadowTarget.texture, Temp.data);
    ...
    if (!ShadowMode){
        ...
        BeginMode3D(OmegaTechData.ShadowCamera);
        UpdateCamera(&OmegaTechData.ShadowCamera, CAMERA_FREE);
        ...
        for (int i = 0; i <= CachedModelCounter; i++) {
            ...
            switch (CachedModels[i].ModelId) {
                case 2:
                    if (S <= 180)DrawModelEx(WDLModels.Model2, {X, Y, Z}, {0, Rotation, 0}, Rotation, {S, S, S}, WHITE);
                    break;
            }
        }
    }
